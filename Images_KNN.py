...
get_ipython().run_cell_magic('time', '', "clf = neighbors.KNeighborsClassifier(metric='manhattan', weights='uniform', n_neighbors=1 )\nclf.fit(data_train, label_train)\nprint(clf)")
...
train_scores, test_scores = validation_curve(
    neighbors.KNeighborsClassifier(metric='manhattan', weights='distance'), data_train, label_train, param_name="n_neighbors", param_range=param_range,
...
plt.title("Validation Curve with KNN on the Image Data")
...
plt.figure()
plt.title("Learning curve with KNN on the Image Data")
plt.xlabel("Training examples")
plt.ylabel("Score")

train_sizes = np.linspace(.1, 1.0, 10)
train_sizes, train_scores, test_scores = learning_curve(neighbors.KNeighborsClassifier(n_neighbors=6, metric='manhattan', weights="distance"), data_train, label_train, cv=5, n_jobs=4, train_sizes=train_sizes)
